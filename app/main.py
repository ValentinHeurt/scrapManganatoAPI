...
def search_multiple_input(textSearch: str | None = Header(default=""),
    ...
    if ig != None:
        ...
        for includedGenre in ig:
            url = f"{url}_{[genre['id'] for genre in genresList if genre['genre'] == includedGenre][0]}"
        url = f"{url}_"

    if eg != None:
        url = f"{url}&g_e="
        for excludedGenre in eg:
            url = f"{url}_{[genre['id'] for genre in genresList if genre['genre'] == excludedGenre][0]}"
        url = f"{url}_"
